...
namespace LinearRegression
{
    ...
    class Program
    {
        ...
        static void TestDifferentInferenceAlgos()
        {
			...
			// lower learning rates are better because they prevent over-fitting but higher learning rate ensures faster learning.
			// the goal of using meta-inference here is to fix a prior over the alpha and maximize the likelihood of the prior while
			// ensuring that a correctness condition is met: error gets below a threshold within n iterations.

			Weighted<double>[] alpha_array= new Weighted<double>[new Weighted(0.1, 0.1), new Weighted(0.01, 0.2), new Weighted(0.0001, 0.7)];
			
			var alphas = new FiniteEnumeration<double> (new Weighted<double>[0.1, 0.01, 0.001, 0.0001]);
